...
input_file <- sprintf("taxi.json")

dataj <- fromJSON(file = input_file)

means_modularity <- dataj$"modularity (1.0)"$mean[1:(max_epoch / time_interval)]
sd_modularity <- dataj$"modularity (1.0)"$std_dev[1:(max_epoch / time_interval)] / sqrt(n_runs)
...
axis(1, at = c(0, 100, 200, 300, 400, 500), labels = TRUE, cex.axis = 1.7)
axis(2, at = c(-0.3, 0.0, 0.3, 0.6, 0.9), labels = TRUE, cex.axis = 1.7)
title(main = "Taxi", cex.main = 2.0, font.main = 1)
...
#legend(x = 156, y = 0.68, legend = c("Modularity", "Primitive", "Label Prop.", "Edge Bet.", "Node Bet.", "Eigenoptions", "Xu et al."),
