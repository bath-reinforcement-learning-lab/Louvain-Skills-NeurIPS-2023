...
### get the data
dataj <- fromJSON(file="stg_scaling.json")
stg_sizes <- dataj$`STG Sizes`
depths <- dataj$`Hierarchy Depths`
######

if (dosave) pdf(paste(path,"/depth_scaling", ".pdf",sep=""), width = 5, height = 3.75)
par(family="serif", mar=c(3.5, 3.5, 0, 0) + 0.5, mgp=c(2.5, 1, 0))
plot.new()
plot.window(ylim=c(5, 8.2), xlim=c(1000, 1200000), log="x") 
...
title(ylab="Hierarchy Depth", cex.lab=2.0)
box()

points(stg_sizes, depths + 1, col = "blue", pch = 16, xpd=T)
lines(stg_sizes, depths + 1, col = "blue", xpd=T)
