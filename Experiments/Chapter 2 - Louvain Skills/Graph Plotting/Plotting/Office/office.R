...
means_xu <- dataj$"level 4"$mean[1:(max_epoch / time_interval)]
sd_xu <- dataj$"level 4"$std_dev[1:(max_epoch / time_interval)] / sqrt(n_runs)

means_leiden <- dataj$"leiden"$mean[1:(max_epoch / time_interval)]
sd_leiden <- dataj$"leiden"$std_dev[1:(max_epoch / time_interval)] / sqrt(n_runs)
...
cip_y_leiden <- c(means_leiden - sd_leiden, rev(means_leiden + sd_leiden))
...
polygon(cip_x, cip_y_leiden, col = adjustcolor("darkred", alpha.f = 0.3), border = FALSE)
...
points(x, means_lab_prop, col = "forestgreen", pch = 16, cex=0.6)

lines(x, means_edge_bet, col = "blue")
points(x, means_edge_bet, col = "blue", pch = 16, cex=0.6)

lines(x, means_node_bet, col = "navy")
points(x, means_node_bet, col = "navy", pch = 16, cex=0.6)

lines(x, means_eig, col = "orange")
points(x, means_eig, col = "orange", pch = 16, cex=0.6)

lines(x, means_xu, col = "darkviolet")
points(x, means_xu, col = "darkviolet", pch = 16, cex=0.6)

lines(x, means_leiden, col = "darkred")
points(x, means_leiden, col = "darkred", pch = 16, cex=0.6)

lines(x, means_louvain, col = "red")
points(x, means_louvain, col = "red", pch = 16, cex=0.6)

# If it's the final plot, add the legend
# legend(x = 25, y = 0.8, legend = c("Louvain", "Leiden", "Primitive", "Label Prop.", "Edge Bet.", "Node Bet.", "Eigenoptions", "Xu et al."),
#        col = c("red", "darkred", "grey30", "forestgreen", "blue", "navy", "orange", "darkviolet"), pch = 16,
